...
test_vcl_file() {
	...
	# echo "lwp-request -m PUT "http://${PASS}@localhost:${AGENT_PORT}/vcl/test3 "<$VCL" > /dev/null"
	# curl -s -o /dev/null -X PUT "http://${PASS}@localhost:${AGENT_PORT}/vcl/test3" <$VCL > /dev/null
	if [ "x$?" = "x0" ]; then pass; else fail "VCL step 1: $OUT"; fi
	inc
	# lwp-request -m GET "http://${PASS}@localhost:${AGENT_PORT}/vcl/test3" >$OUT
	curl -s -X GET "http://${PASS}@localhost:${AGENT_PORT}/vcl/test3" >$OUT
